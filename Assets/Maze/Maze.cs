...
public class Maze : MonoBehaviour {
	...
	void Awake(){ 
		if (instance != null) Destroy(this); // Don't create this maze.
		instance = this;
	}
		...
		for(int i = 0; i < width; i++){
			for (int j = 0; j < height; j++){
				if(maze[i, j] == "w"){
					...
					go.GetComponent<Renderer>().material = wallMaterials[index];
				} else if(maze[i,j] == "p"){ 
	...
	void CreateBarriers (int[] exits)
	{
		...
		for(int i = 0; i < width; i++){
			...
			if(i == exits[0] && exits[0] != -1){
				...
				int index = r.Next(wallMaterials.Length);
				b1.GetComponent<Renderer>().material = wallMaterials[index];
			} else {
			...
			if(i == exits[2] && exits[2] != -1){
				...
				int index = r.Next(wallMaterials.Length);
				b2.GetComponent<Renderer>().material = wallMaterials[index];
			} else {
	...
	void PlaceFloor ()
	{
		Vector3 scale = new Vector3(width * wall.GetComponent<Renderer>().bounds.size.x+10, 0f, 
		                            height * wall.GetComponent<Renderer>().bounds.size.z+10);
		Vector3 position = (scale/2f) - wall.GetComponent<Renderer>().bounds.size;
		position.y = 0;
	...
	public GameObject InstantiateObject(GameObject obj, int i, int j){
		GameObject go = (GameObject) Instantiate(
			obj,
			new Vector3(i * wall.GetComponent<Renderer>().bounds.size.x, 
		            wall.GetComponent<Renderer>().bounds.size.y / 2, 
		            j * wall.GetComponent<Renderer>().bounds.size.z), 
					floor.transform.rotation);
		return go;
	}

	public Vector3 GetMazePosition(int i, int j){
		return new Vector3(i * wall.GetComponent<Renderer>().bounds.size.x, 
		                   wall.GetComponent<Renderer>().bounds.size.y / 2, 
		                   j * wall.GetComponent<Renderer>().bounds.size.z);
	}
